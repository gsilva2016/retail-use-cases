...
PROMPT='As an expert investigator, please analyze this video depicting a retail checkout scenario. Summarize the video, highlighting any shoplifting or suspicious activity. 
The output must contain the following 3 sections:
Overall Summary, Activity Observed, Potential Suspicious Activity.
It should be formatted similar to the following example:
...
PYTHONPATH=. python summarizer/video_summarizer.py $INPUT_FILE $MODEL -d $DEVICE -r $RESOLUTION_X $RESOLUTION_Y -p "$PROMPT"
